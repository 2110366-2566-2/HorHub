...
function DormList() {
  ...
  useEffect(() => {
    if (
      sorter === "nearest" &&
      (currentLatitude === -999 || currentLongitude === -999)
    ) {
  ...
  useDebounce(
    ...
    [name, location, dormFacilities, maxPrice, minPrice],
    500
  );
  ...
  return (
    <div className="min-h-[calc(100vh-7rem)]">
      ...
      {dormsData.filter((data) => {
        ...
        <div className="w-full h-full flex flex-col justify-center items-center py-4 text-base font-bold">
          No dorm found...
      ...
      <ul className="grid max-w-[26rem] sm:max-w-[52.5rem] lg:max-w-7xl w-[100%] mt-[2rem] mb-[4rem] grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mx-auto gap-6 lg:gap-y-8 xl:gap-x-8  px-4 sm:px-6 lg:px-8">
        {dormsData
          .sort((a: Dorm, b: Dorm) => {
            if (sorter === "cheapest") {
              const minPriceA = Math.min.apply(
                null,
                a.roomTypes.map((value) => value.cost)
              );
              const minPriceB = Math.min.apply(
                null,
                b.roomTypes.map((value) => value.cost)
              );
              ...
              const maxPriceA = Math.max.apply(
                null,
                a.roomTypes.map((value) => value.cost)
              );
              const maxPriceB = Math.max.apply(
                null,
                b.roomTypes.map((value) => value.cost)
              );
