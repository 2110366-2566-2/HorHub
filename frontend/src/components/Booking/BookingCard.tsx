...
const BookingCard = ({ data }: { data: BookingType }) => {
    ...
    async function cancelReservation() {
        ...
        try {
            const res = await fetch(process.env.REACT_APP_BACKEND_URL + "/bookings/" + data.id, {
                method: "DELETE",
                credentials: "include"
            });
                ...
                <div className="flex items-center">
                    {
                        ...
                        <button 
                          ...
                          onClick={()=>{
                              if (document) {
                                  (document.getElementById('delete_booking_' + data.id) as HTMLFormElement).showModal();
                              }
                          }}>
                          Cancel
                        </button>
                        <dialog id={'delete_booking_' + data.id} className="modal">
                            <div className="modal-box bg-white gap-3 space-y-3">
                                <div className="font-bold text-lg text-center">Cancelling Reservation</div>
                                <div className="text-sm">Are you sure to cancel this reservation?</div>
                                <div className="w-full flex justify-center gap-2">
                                <form method="dialog">
                                    <button className="bordered-button">No</button>
                                </form> 
                                {
                                    (allowSubmit) ? <button className="danger-button" onClick={cancelReservation}>Yes</button>
                                    : <button className="disabled-button" disabled>Yes</button>
                                }
                                </div>
                            </div>
