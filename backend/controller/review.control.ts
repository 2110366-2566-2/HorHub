...
export const getReview = async (req: Request, res: Response) => {
  ...
  try {
    if (reviewId.length != 24 || /[0-9A-Fa-f]{24}/g.test(reviewId) === false) {
      return res
        .status(404)
        .send(generateStatusResponse(404, "No review found"));
    }
...
export const updateReview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!parseStatus.success)
      return res.status(400).send(parseStatus.error.message);
      ...
      return res
        ...
        .send(
          generateStatusResponse(
            403,
            "You don't have access to manage this review"
          )
        );
    }
  ...
  try {
    ...
    if (!findBooking) {
      return res
        .status(403)
        .send(
          "You haven't book this dorm or there is no booking that is in completed status"
        );
    }
