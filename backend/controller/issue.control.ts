...
const adminUpdateIssueSchema = z.object({
    resolveMessage: z.string().trim().min(1, { message: "Please fill resolve message" })
})
...
export const getIssue = async (req: Request, res: Response) => {
    ...
    try {
        if (issueId.length != 24 || /[0-9A-Fa-f]{24}/g.test(issueId) === false) {
            return res.status(404).send(generateStatusResponse(404, "No issue found"));
        }
        ...
        if (user.role !== "Admin" && issueRes.userId !== user.id) {
            return res.status(403).send(generateStatusResponse(403, "You don't have permission to view this issue"))
        }
...
export const updateIssue = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (findIssueRes.userId !== user.id) {
            return res.status(403).send(generateStatusResponse(403, "You don't have permission to edit this issue"))
        }
...
export const deleteIssue = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (findIssueRes.userId !== user.id) {
            return res.status(403).send(generateStatusResponse(403, "You don't have permission to delete this issue"))
        }
...
export const resolveIssue = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (findIssueRes.status !== "Waiting") {
            return res.status(403).send(generateStatusResponse(403, "This issue had already been resolved or rejected"))
        }
